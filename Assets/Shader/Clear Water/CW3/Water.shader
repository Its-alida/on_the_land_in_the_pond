	...
	) : SV_TARGET
{
	...
	// Edge fade
	// Technically, the edge of water doesn't fade. But reflection probes and etc are inaccurate. 
	...
	float4 refrPos = UnityWorldToClipPos(float4(worldPos + (i.normal.xyz - pix.worldNormal) * 0.1, 1.0));
...
#if defined(_USE_SSR)
	col = evaluateScreenSpaceReflections(reflectionDir, worldPos, getSSRParams(), i.pos.xy);
